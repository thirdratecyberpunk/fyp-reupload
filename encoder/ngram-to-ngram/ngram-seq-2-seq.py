#!/usr/bin/python
'''
Sequence to sequence example in Keras (ngram-level).
'''
...
parser = argparse.ArgumentParser(description='ngram level sequence to sequence model trained on ENGLISH-FRENCH model.')
...
'''
Converts a sequence of tokens into a numpy array.
:param input: numpy array to convert
:returns: a string representation of the converted translation
'''
def encode_string_to_sequence(input_string):
    ...
    for count, ngram in enumerate(input_string):
        ...
        try:
            converted_string[count] = encoder_input_data[input_token_index[tuple(ngram)]]
        except KeyError:
            random_ngram = (random.choice(list(input_token_index)[:10000]))
            converted_string[count] = encoder_input_data[input_token_index[random_ngram]]
    # return the combined numpy arrays
    return converted_string

'''
Converts a numpy array into a sequence of tokens.
:param input: numpy array to convert
:returns: a string representation of the converted translation
'''
...
def decode_sequence(input_seq):
    ...
    while not stop_condition:
        ...
        sampled_ngram = reverse_target_word_index[sampled_token_index]
        # adds that word to the decoded sentence
        decoded_sentence.append(sampled_ngram)

        # Exit condition: either hit max length or find stop word.
        if ('\n' in sampled_ngram or len(decoded_sentence) > max_decoder_seq_length):
            stop_condition = True
...
with open(sample_input_location, 'r', encoding='utf-8') as f:
    ...
    for item in lines:
        ...
        item_as_ngrams = textinformation.ngrams(item, 3)
        input_seq = encode_string_to_sequence(item_as_ngrams)
        decoded_ngrams = decode_sequence(input_seq)
        decoded_sentence = textinformation.ngram_to_sentence(decoded_ngrams)
